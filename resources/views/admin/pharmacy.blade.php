...
@section('content')
    ...
    {{--edit modal for Pharmacy--}}
    <div class="modal" id="editModal" tabindex="-1" role="dialog" aria-labelledby="Survey">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title" id="exampleModalLabel1">Edit Pharmacy</h4>
                    <button type="button" class="close ml-auto" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <form  class="form-horizontal"  method="post" action="{{route('admin.pharmacy.update')}}" id="editForm">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <div class="edit-result">
                            ...
                            <div class="form-group row">
                                <label for="edit-name" class="control-label col-sm-3">Pharmacy Name</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row">
                                <label for="edit-location" class="control-label col-sm-3">Pharmacy Location</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-location" name="location" required>
                                    <span class="text-danger" id="elocation" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-description" class="control-label col-sm-3">Pharmacy Description</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-description" name="description" required>
                                    <span class="text-danger" id="edescr" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-latitude" class="control-label col-sm-3">Pharmacy Latitude</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-latitude" name="latitude" required>
                                    <span class="text-danger" id="elatitude" style="color: red"></span>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="edit-longitude" class="control-label col-sm-3">Pharmacy Longitude</label>
                                <label class="col-sm-1 control-label">:</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="edit-longitude" name="longitude" required autofocus>
                                    <span class="text-danger" id="elongitude" style="color: red"></span>
                                </div>
                            </div>
                        </div>
            ...
            manageTable.on('click', '.js-edit', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#edit-name").val(response.pharmacy.name);
                        $("#edit-location").val(response.pharmacy.location);
                        $("#edit-description").val(response.pharmacy.description);
                        $("#edit-latitude").val(response.pharmacy.latitude);
                        $("#edit-longitude").val(response.pharmacy.longitude);

                        // add hidden id
                        $('#id').val(response.pharmacy.id);
                        // update - form
...
//                                form[0].reset();
                                ...
                                $('#edit-messages').html('<div class="alert alert-success">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Pharmacy  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-delete', function () {
                var button = $(this);
                bootbox.confirm("Are you sure you want to Delete this Pharmacy?", function (result) {
                    if (result) {
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                bootbox.alert({
                                    title: "Error",
                                    message: "<i class='fa fa-warning'></i>" +
                                        " Pharmacy not Delete please try again"
                                });
                            }
                        });
